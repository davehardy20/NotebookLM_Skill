...
export const CacheEntrySchema = z.object({
  ...
  notebookUrl: z.string().url().describe('URL of the notebook queried'),
  timestamp: z.number().describe('Unix timestamp when entry was cached'),
  hitCount: z.number().int().nonnegative().default(0).describe('Number of times this entry was retrieved'),
});
...
export const CacheStatsSchema = z.object({
  ...
  evictions: z.number().int().nonnegative().default(0).describe('Total entries evicted due to size limit'),
